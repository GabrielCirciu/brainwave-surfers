...
def main():
    ...
    # Plotting Boxplots
    fig, (ax_acc, ax_auc) = plt.subplots(1, 2, figsize=(13, 6))
    ...
    for metric_key, ax, title in metrics:
        ...
        for patch, pipe in zip(bp['boxes'], pipelines):
            if pipe.startswith('cov'):
                c = 'forestgreen'
            elif pipe.startswith('csp'):
                c = 'dodgerblue'
            else:
                c = 'darkorange'
        ...
        custom_lines = [
            Patch(facecolor='forestgreen', alpha=0.7, label='Covariance'),
            Patch(facecolor='dodgerblue', alpha=0.7, label='CSP'),
            Patch(facecolor='darkorange', alpha=0.7, label='Augmented'),
